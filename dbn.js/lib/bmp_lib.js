define(function (require, exports, module) {
   ...
   * @param {Array} optPalette Optional palette data.
   * @throws {string} If the element is invalid.
   */
  bmpLib.render = function(element, grid, optPalette) {
    if (typeof element == 'string') {
      element = document.getElementById(element);
    }
    if (!element || !element.tagName) {
      throw('bmpLib.render: Invalid element: ' + element);
    } else if (element.tagName == 'IMG') {
      element.src = this.imageSource(grid, optPalette);
    } else {
      throw('bmpLib.render: Invalid HTML tag: ' + element.tagName);
    }
  };
   ...
   */
  bmpLib.imageSource = function(grid, optPalette) {
    var a = this.normalize(grid, optPalette);
    var data = this.createBmp(a[0], a[1]);
    return 'data:image/bmp;base64,' + this.encode64(data);
  };
   ...
   * @private
   */
  bmpLib.normalize = function(grid, optPalette) {
    var palette;
    // Check what type of data was provided.
    if (grid.length === 0) {
      // 0x0 picture.
      palette = null;
    } else if (typeof grid[0] == 'string' && optPalette) {
      ...
      grid = this.arrayArrayToArrayStr(grid);
      palette = optPalette;
   ...
   * @private
   */
  bmpLib.createBmp = function(grid, palette) {
    ...
    var biHeight = this.multiByteEncode(height, 4);
    var biWidth = this.multiByteEncode(width, 4);
    var bfOffBits = this.multiByteEncode(40, 4);
    var bitCount;
    if (palette && palette.length <= 256) {
      ...
      if (palette) {
        ...
        grid = this.depalette(grid, palette);
      }
      palette = null;
    }
    var biBitCount = this.multiByteEncode(bitCount, 2);
    ...
    if (bitCount != 24) {
      ...
      if (bmpLib.createBmp.paletteStrCache == paletteStr) {
        // The previously computed palette was identical.  Use it.
        rgbQuad = bmpLib.createBmp.rgbQuadCache;
      } else {
        rgbQuad = [];
        var r = 0;
        var g = 0;
        ...
        bmpLib.createBmp.paletteStrCache = paletteStr;
        bmpLib.createBmp.rgbQuadCache = rgbQuad;
      }
    } else {
      rgbQuad = '';
    }
    ...
    bitmap = bitmap.replace(/yyyy/, this.multiByteEncode(
        bitmapFileHeader.length + bitmapInfoHeader.length + rgbQuad.length, 4));
    // Insert the size of the bitmap in bytes.
    bitmap = bitmap.replace(/xxxx/, this.multiByteEncode(bitmap.length, 4));
    return bitmap;
  };

  // Cached palette to avoid recomputing identical palettes.
  bmpLib.createBmp.paletteStrCache = '';
  bmpLib.createBmp.rgbQuadCache = '';
   ...
   * @private
   */
  bmpLib.multiByteEncode = function(number, bytes) {
   ...
   * @private
   */
  bmpLib.arrayArrayToArrayStr = function(arrayArray) {
    var arrayStr = new Array(arrayArray.length)
      , line
      ;
   ...
   * @private
   */
  bmpLib.depalette = function(oldGrid, palette) {
   ...
   * @private
   */
  bmpLib.dec2hex = function(decimal) {
    var a = decimal % 16;
    var b = (decimal - a) / 16;
    return bmpLib.dec2hex.hexChars.charAt(b) +
           bmpLib.dec2hex.hexChars.charAt(a);
  };
  bmpLib.dec2hex.hexChars = '0123456789ABCDEF';
   ...
   * @private
   */
  bmpLib.encode64 = function(input) {
    var output = '';
    var i = 0;
    ...
    do {
      ...
      output = output + bmpLib.encode64.keyStr.charAt(enc1) +
          bmpLib.encode64.keyStr.charAt(enc2) + 
          bmpLib.encode64.keyStr.charAt(enc3) +
          bmpLib.encode64.keyStr.charAt(enc4);
    } while (i < input.length);

    return output;
  };
  bmpLib.encode64.keyStr = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=';
  ...
  bmpLib.scale = function(image, factor) {
    var oldWidth = image[0].length;
    var oldHeight = image.length;
    ...
    var iF // row factor counter
      , jF // column factor counter
      ;
    ...
    for (i = 0; i < oldHeight; i++) {
      ...
      // do factor times
      or = image[i];
      for (iF = 0; iF < factor; iF++) {
        nr = [];
        for (j = 0; j < oldWidth; j++) {

          // do factor times
          for (jF = 0; jF < factor; jF++) {
            nr.push(or[j]);
          }
        }
